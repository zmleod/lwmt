...
$query_img = "SELECT * FROM neusaltspray_exp_img where baogao_bianhao = '$baogao_bianhao'";
// 执行查询
  ...
  <table   class="head_table songti" width="698">
    <tr class="noline_tr">
      <td class="left_td" width="210px">报告编号：<? echo $baogao_bianhao;?> </td>
           ...
           <td class="right_td"><a href="./report_edit_neusaltspray.php?baogao_bianhao=<? echo $baogao_bianhao;?>">修改报告</a> </td>
         <td class="right_td"> <a href="./report_img_upload_neusaltspray.php?baogao_bianhao=<? echo $baogao_bianhao;?>">增加图片</a>  </td>
  ...
  <div >
    <table  class="songti" align="center" width="698">
       <tr>
        ...
        <td style="width:16.6%" colspan="2">试验日期</td>
      ...
      <tr>
        <td>试验项目</td>
        <td colspan="2" ><?echo $get_row['shiyan_xiangmu'];?></td>
        <td colspan="2" >试验设备</td>
        <td><?echo $get_row['shiyan_shebei'];?></td>
        <td colspan="2" >试验条件</td>
        <td colspan="2" ><?echo $get_row['shiyan_tiaojian'];?></td>
      </tr>
      <tr>
        <td>试验标准</td>
        <td colspan="6"><?echo $get_row['shiyan_biaozhun'];?></td>
           <td>样品数量</td>
    ...
    <table  class="songti" align="center" width="698">
        ...
        <td style="width:11.1%">样品名称</td>
              ...
              <td style="width:11.1%">样品编号</td>
                      <td style="width:11.1%">测试时间(h)</td>
                      <td style="width:11.1%">腐蚀前质量(g)</td>
                      <td style="width:11.1%">去除腐蚀产物质量(g)</td>
                      <td style="width:11.1%">失重量(g)</td>
                      <td>平均腐蚀速率(mg/cm<sup>2</sup>/day)</td>
      </tr>
      ...
      while ( $row = mysql_fetch_assoc( $result ) ) {
        ...
        <td>' . $row[ 'shiyan_shijian' ] . '</td>
        <td>' . $row[ 'zhongliang_fushiqian' ] . '</td>
        <td>' . $row[ 'zhongliang_fushihou' ] . '</td>
...
function imgconfirm(message, param) {
  ...
  if (result) {
    // 用户点击了"确定"
    // 可以在这里执行确认后的操作
    window.location.href = '../action/img_delete_neusaltspray.php?baogao_bianhao=<?php echo $baogao_bianhao;?>&id='+param;
  } else {
    // 用户点击了"取消"
    // 可以在这里执行取消后的操作
    console.log("用户点击了取消。");
  ...
  if (result) {
    // 用户点击了"确定"
    // 可以在这里执行确认后的操作
    window.location.href = '../action/report_delete_by_baogao_bianhao_neusaltspray.php?baogao_bianhao=<? echo $baogao_bianhao;?>';
  } else {
    // 用户点击了"取消"
    // 可以在这里执行取消后的操作
    console.log("用户点击了取消。");
